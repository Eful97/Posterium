  ...
  const openPosterBrowser = async (item: SearchResult) => {
    ...
    try {
      ...
      const [details, rankData, awardData] = await Promise.all([
        ...
        http<{ awards: string[]; nominations: string[]; studios: string[]; franchise: string | null; basedOn: string | null; director: string | null; keywords: string[] }>(`/api/awards/${itemType}/${itemId}?api_key=${encodeURIComponent(tmdbKey)}`, { timeout: 15000 }).catch(() => ({ awards: [] as string[], nominations: [] as string[], studios: [] as string[], franchise: null, basedOn: null, director: null, keywords: [] as string[] })),
      ])
