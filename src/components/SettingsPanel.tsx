...
import { saveDefaults, resetDefaults } from "@/lib/save-defaults"
import { SliderRow } from "@/components/SliderRow"
import { Toggle } from "@/components/Toggle"
import { BadgeStyleSelector, SecretInput, MenuItem } from "@/components/ui"
import { Star, Trophy, Palette, Ruler, Cloud, Minus, Circle, Save, Check, Upload, Download, Clipboard, Trash2, Key, Sparkles, Tv, Flame, Moon, Sun, RotateCcw } from "lucide-react"
...
/** Sezione a card con titolo e hairline accent (da prototipo Open Design). */
function SettingsSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="relative rounded-xl border border-white/[0.06] bg-white/[0.015] px-3 pt-2.5 pb-1.5">
      <span className="absolute top-0 left-3 right-3 h-px bg-gradient-to-r from-transparent via-accent-orange/60 to-transparent" aria-hidden="true" />
      <h3 className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted mb-2">{title}</h3>
...
export function SettingsPanel({ tmdbKeyInput, setTmdbKeyInput, setTmdbKey, setSettingsOpen, exportData, importData, mdblistApiKey, setMdblistApiKey, mobile }: Props) {
  ...
  const content = (
    <div className="space-y-3">
      <SettingsSection title={t("ui.sectionApiKeys")}>
      ...
      <SettingsSection title={t("ui.sectionBadgeDefaults")}>
        <Row label={t("ui.genreRatingBadge")} icon={<Star />}>
          <Toggle value={ed.defaultGlobalBadges} onChange={ed.setDefaultGlobalBadges} />
        </Row>
        <div className="pl-4 space-y-1.5 border-l border-surface2/60 ml-1.5">
          <Row label={t("ui.badgeGenre")}><Toggle value={ed.defaultBadgeGenre} onChange={ed.setDefaultBadgeGenre} /></Row>
          <Row label={t("ui.badgeYear")}><Toggle value={ed.defaultBadgeYear} onChange={ed.setDefaultBadgeYear} /></Row>
          <Row label={t("ui.badgeRating")}><Toggle value={ed.defaultBadgeRating} onChange={ed.setDefaultBadgeRating} /></Row>
        </div>
        <Row label={t("ui.trendBadge")} icon={<Trophy />}>
          <Toggle value={ed.defaultRankingBadges} onChange={ed.setDefaultRankingBadges} />
        </Row>
        <Row label={t("ui.networkLogo")} icon={<Tv />}>
          <Toggle value={ed.defaultNetworkLogo} onChange={(v) => { ed.setDefaultNetworkLogo(v); ed.setNetworkLogo(v) }} />
        </Row>
        <Row label={t("ui.badgePosition")} icon={<Flame />}>
        ...
        <label className="text-[11px] text-muted font-medium flex items-center gap-1.5 pt-1"><Circle className="w-3 h-3" /> {t("ui.styleRankingDefault")}</label>
        ...
        <label className="text-[11px] text-muted font-medium flex items-center gap-1.5 pt-1"><Palette className="w-3 h-3" /> {t("ui.styleDefault")}</label>
      ...
      <SettingsSection title={t("ui.sectionBlurDefaults")}>
        <Row label={t("ui.blurDefault")}>
      ...
      <SettingsSection title={t("ui.sectionExtra")}>
        <Row label={t("ui.autoRotateDefault")} icon={<RotateCcw />}>
          <Toggle value={ed.defaultAutoRotateClean} onChange={ed.setDefaultAutoRotateClean} />
        </Row>
        <Row label={t("ui.uiAccentDynamic")} icon={<Palette />}>
          <Toggle value={uiAccent} onChange={setUiAccent} />
        </Row>
        <Row label={t("ui.logoFitEnabled")} icon={<Sparkles />}>
          <Toggle value={ed.defaultLogoFitEnabled} onChange={ed.setDefaultLogoFitEnabled} />
        </Row>
        <Row label={t("ui.sectionTheme")} icon={theme === "light" ? <Sun /> : <Moon />}>
          <div className="flex gap-1 flex-1 max-w-[150px]">
            <button
              type="button"
            ...
            >{t("ui.themeLight")}</button>
          </div>
        </Row>
      </SettingsSection>

      <SettingsSection title={t("ui.sectionData")}>
        ...
        <button type="button" onClick={() => { resetDefaults(ed); setResetSaved(true); setTimeout(() => setResetSaved(false), 1500) }} className="w-full text-center text-xs font-semibold py-2 rounded-lg bg-white/[0.06] text-muted border border-white/10 hover:text-zinc-200 hover:bg-white/[0.1] active:scale-[0.98] transition-all duration-150"><span className="flex items-center gap-1.5 justify-center">{resetSaved ? <><Check className="w-3 h-3" /> {t("ui.saved")}</> : <><RotateCcw className="w-3 h-3" /> {t("ui.resetDefaults")}</>}</span></button>
        <div className="pt-1">
      ...
      <SettingsSection title={t("ui.cacheDiagnostics")}>
        <div className="flex items-center justify-between text-[11px] font-medium text-muted px-1">
          <span className="flex items-center gap-1.5"><Flame className="w-3 h-3 text-amber-400" /> {t("ui.warmup")}</span>
          <span className="text-zinc-500 text-[10px] font-mono tabular-nums">{cacheCount !== null ? `${cacheCount} ${cacheCount === 1 ? t("ui.cacheEntryOne") : t("ui.cacheEntryMany")}` : ""}</span>
        </div>
        <div className="grid grid-cols-2 gap-1.5">
          ...
          }} className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-lg text-[11px] font-medium bg-amber-500/10 text-amber-300 hover:bg-amber-500/20 active:scale-[0.98] transition-all"><Flame className="w-3 h-3" /> {t("ui.warmup")}</button>
  ...
  return (
    <div className="absolute right-0 top-full mt-2 bg-black/85 backdrop-blur-xl border border-border/50 rounded-xl p-3 shadow-2xl shadow-black/50 z-50 min-w-[300px] max-h-[80vh] overflow-y-auto animate-fade-scale-in" onClick={(e) => e.stopPropagation()}>
      {content}
    </div>
  )
}
