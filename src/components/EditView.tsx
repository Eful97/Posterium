  ...
  const logoCol = (
    <div className="w-full md:w-64 shrink-0 self-start md:sticky md:top-4 animate-fade-scale-in md:order-3" style={{ animationDelay: "120ms", animationFillMode: "backwards" }}>
      ...
      <div className="space-y-1 mt-2">
        ...
        <SliderRow icon="📏" label={p.t("ui.height")} value={p.gradientHeight} min={5} max={100} boundsMin={5} boundsMax={100} onChange={(v) => { p.setGradientHeight(v); localStorage.setItem("gradient_height", String(v)) }} onDoubleClick={() => { p.setGradientHeight(5); localStorage.setItem("gradient_height", "5") }} editingValue={p.editingValue} editText={p.editText} setEditingValue={p.setEditingValue} setEditText={p.setEditText} editingKey="gradHeight" suffix="%" />
        <SliderRow icon="🌫️" label={p.t("ui.fade")} value={p.gradientFade} min={0} max={100} boundsMin={0} boundsMax={100} onChange={(v) => { p.setGradientFade(v); localStorage.setItem("gradient_fade", String(v)) }} onDoubleClick={() => { p.setGradientFade(32); localStorage.setItem("gradient_fade", "32") }} editingValue={p.editingValue} editText={p.editText} setEditingValue={p.setEditingValue} setEditText={p.setEditText} editingKey="gradFade" suffix="%" />
        <SliderRow icon="〰️" label="Sfumatura" value={p.gradientFadeWidth} min={0} max={100} boundsMin={0} boundsMax={100} onChange={(v) => { p.setGradientFadeWidth(v); localStorage.setItem("gradient_fade_width", String(v)) }} onDoubleClick={() => { p.setGradientFadeWidth(100); localStorage.setItem("gradient_fade_width", "100") }} editingValue={p.editingValue} editText={p.editText} setEditingValue={p.setEditingValue} setEditText={p.setEditText} editingKey="gradFadeWidth" suffix="%" />
      </div>
    </div>
  )
