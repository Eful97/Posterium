      ...
      <div className="mt-3">
        ...
        <div className="flex gap-1">
          {(["shadow","pill","outline","bar"] as const).map(s => (
            <button key={s} onClick={() => { p.setBadgeStyle(s); p.setDefaultBadgeStyle(s); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), badgeStyle: s })) }} className={`flex-1 px-2 py-1.5 text-[11px] font-semibold rounded-lg transition-all duration-150 ${p.badgeStyle === s ? "bg-white/20 text-white shadow-sm" : "bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-zinc-200"}`}>{s === "shadow" ? "Ombra" : s === "pill" ? "Pill" : s === "outline" ? "Outline" : "Barra"}</button>
          ))}
        </div>
      </div>
      <div className="space-y-1 mt-2">
        <div className="flex gap-2 items-center mb-1">
          <button onClick={() => { p.setBlurEnabled(!p.blurEnabled); p.setDefaultBlurEnabled(!p.blurEnabled); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), blurEnabled: !p.blurEnabled })) }} className={`flex-1 py-2 text-[12px] font-bold rounded-lg transition-all duration-150 ${p.blurEnabled ? "bg-white/15 text-white shadow-sm" : "bg-white/5 text-zinc-400 hover:bg-white/10"}`}>{p.blurEnabled ? "🌫️ Sfocatura attiva" : "🌫️ Sfocatura disattivata"}</button>
        </div>
        {p.blurEnabled && <><SliderRow icon="📏" label="Altezza blur" value={p.gradientHeight} min={5} max={100} boundsMin={5} boundsMax={100} onChange={(v) => { p.setGradientHeight(v); p.setDefaultGradientHeight(v); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), gradientHeight: v })) }} onDoubleClick={() => { p.setGradientHeight(30); p.setDefaultGradientHeight(30); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), gradientHeight: 30 })) }} editingValue={p.editingValue} editText={p.editText} setEditingValue={p.setEditingValue} setEditText={p.setEditText} editingKey="gradHeight" suffix="%" />
        <SliderRow icon="🌫️" label="Intensità blur" value={p.blurIntensity} min={1} max={50} boundsMin={1} boundsMax={50} onChange={(v) => { p.setBlurIntensity(v); p.setDefaultBlurIntensity(v); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), blurIntensity: v })) }} onDoubleClick={() => { p.setBlurIntensity(5); p.setDefaultBlurIntensity(5); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), blurIntensity: 5 })) }} editingValue={p.editingValue} editText={p.editText} setEditingValue={p.setEditingValue} setEditText={p.setEditText} editingKey="blurIntensity" suffix="px" />
        <SliderRow icon="〰️" label="Fade blur" value={p.blurFade} min={0} max={100} boundsMin={0} boundsMax={100} onChange={(v) => { p.setBlurFade(v); p.setDefaultBlurFade(v); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), blurFade: v })) }} onDoubleClick={() => { p.setBlurFade(60); p.setDefaultBlurFade(60); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), blurFade: 60 })) }} editingValue={p.editingValue} editText={p.editText} setEditingValue={p.setEditingValue} setEditText={p.setEditText} editingKey="blurFade" suffix="%" />
        <SliderRow icon="🌑" label="Velatura" value={p.blurDarkness} min={0} max={100} boundsMin={0} boundsMax={100} onChange={(v) => { p.setBlurDarkness(v); p.setDefaultBlurDarkness(v); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), blurDarkness: v })) }} onDoubleClick={() => { p.setBlurDarkness(40); p.setDefaultBlurDarkness(40); localStorage.setItem("badgeDefaults", JSON.stringify({ ...JSON.parse(localStorage.getItem("badgeDefaults") || "{}"), blurDarkness: 40 })) }} editingValue={p.editingValue} editText={p.editText} setEditingValue={p.setEditingValue} setEditText={p.setEditText} editingKey="blurDarkness" suffix="%" /></>}
      </div>
    </div>
  )
