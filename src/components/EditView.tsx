      ...
      <div className="bg-zinc-800/80 rounded-2xl overflow-hidden relative shadow-2xl shadow-black/50 backdrop-blur-sm border border-white/[0.07]">
        <div className="relative aspect-[2/3] select-none pointer-events-none bg-zinc-900/50 overflow-hidden rounded-2xl">
          ...
          {p.rankingBadges && (() => {
            ...
            if (isNewMovie) return <div className="absolute inset-0"><ExtraBadge label="Nuovo film" containerW={380} containerH={570} color={p.badgeBgColor} posterPath={p.previewPoster?.file_path} genreName={p.metaInfo.genres[0]?.name || null} /></div>
            if (isNewSeries) return <div className="absolute inset-0"><ExtraBadge label="Nuova serie" containerW={380} containerH={570} color={p.badgeBgColor} posterPath={p.previewPoster?.file_path} genreName={p.metaInfo.genres[0]?.name || null} /></div>
            if (award) return <div className="absolute inset-0"><ExtraBadge label={award} containerW={380} containerH={570} color={p.badgeBgColor} posterPath={p.previewPoster?.file_path} genreName={p.metaInfo.genres[0]?.name || null} /></div>

            if (p.selected?.media_type === "movie" && p.selected?.release_date) {
              const releaseTime = new Date(p.selected.release_date).getTime()
              if (releaseTime < now && now - releaseTime < twoMonths) return <div className="absolute inset-0"><ExtraBadge label="Al cinema" containerW={380} containerH={570} color={p.badgeBgColor} posterPath={p.previewPoster?.file_path} genreName={p.metaInfo.genres[0]?.name || null} /></div>
            }
            ...
            if (p.selected?.media_type === "tv" && p.metaInfo.next_episode_to_air?.air_date) {
              ...
              if (nextAir > now && nextAir - now < twoWeeks) return <div className="absolute inset-0"><ExtraBadge label="Nuova stagione" containerW={380} containerH={570} color={p.badgeBgColor} posterPath={p.previewPoster?.file_path} genreName={p.metaInfo.genres[0]?.name || null} /></div>
            }

            if (p.trendRank) return <div className="absolute inset-0"><RankingBadge rank={p.trendRank} containerW={380} containerH={570} color={p.badgeBgColor} posterPath={p.previewPoster?.file_path} genreName={p.metaInfo.genres[0]?.name || null} /></div>
            ...
            if (extra) return <div className="absolute inset-0"><ExtraBadge label={extra} containerW={380} containerH={570} color={p.badgeBgColor} posterPath={p.previewPoster?.file_path} genreName={p.metaInfo.genres[0]?.name || null} /></div>
            return null
          })()}
