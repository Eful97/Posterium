    ...
    <div className="app-shell text-foreground relative overflow-x-hidden">
      ...
      <div className="hidden md:flex absolute top-4 right-4 z-20">
        ...
        <div className="floating-group relative z-50">
          ...
          <button aria-label={p.t("ui.myPostersBtn")} onClick={() => { if (p.view === "myposters") { window.history.back() } else { window.history.replaceState({ view: "myposters" }, ""); p.setView("myposters") } }} className="h-9 w-9 flex items-center justify-center rounded-lg text-xs font-semibold text-zinc-300 hover:bg-white/[0.08] active:scale-[0.93] transition-all duration-150 press-scale">{p.mappings.length}</button>
          <div className="relative">
            <button aria-label={p.t("ui.settings")} onClick={(e) => { e.stopPropagation(); p.setSettingsOpen((o) => !o) }} className="h-9 w-9 flex items-center justify-center rounded-lg active:scale-90 transition-all duration-150 text-zinc-300 hover:bg-white/[0.08] press-scale"><Settings className="w-4 h-4" /></button>
      ...
      <div className="relative z-10 max-w-[1680px] mx-auto px-4 pt-5 md:pt-[68px] pb-20 md:pb-6 pb-[max(5rem,env(safe-area-inset-bottom)+4rem)]">
        <div className="flex flex-col items-center pb-4 animate-fade-scale-in relative">
          ...
          <div className="flex md:hidden items-center gap-2 flex-wrap justify-center">
            ...
            <button aria-label={p.t("ui.saveProfile")} onClick={() => setProfileModalOpen(true)} className="top-action-button h-9 w-9 flex items-center justify-center bg-white/[0.06] border border-white/10 text-zinc-400 hover:text-zinc-200"><svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="8.5" cy="7" r="4" /><polyline points="17 11 19 13 23 9" /></svg></button>
            ...
            <button aria-label={p.t("ui.myPostersBtn")} onClick={() => { if (p.view === "myposters") { window.history.back() } else { window.history.replaceState({ view: "myposters" }, ""); p.setView("myposters") } }} className="top-action-button h-9 px-2 text-xs font-semibold bg-white/[0.06] border border-white/10 text-zinc-200">{p.mappings.length}</button>
            <button aria-label={p.t("ui.settings")} onClick={() => p.setSettingsOpen(true)} className="top-action-button h-9 w-9 flex items-center justify-center bg-white/[0.06] border border-white/10 text-zinc-200 press-scale"><Settings className="w-4 h-4" /></button>
          </div>
          </>
        </div>
