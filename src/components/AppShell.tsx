    ...
    <div className="min-h-screen bg-background text-foreground relative">
      ...
      <div className="max-w-full px-4 pt-5 md:pt-3 pb-20 md:pb-3">
        <div className="hidden md:flex justify-end items-center gap-2 mb-4">
          ...
          <button onClick={async () => { await navigator.clipboard.writeText(p.urlPattern); setInstalled(true); setTimeout(() => setInstalled(false), 2000) }} disabled={!p.urlPattern} className="h-10 px-3 bg-accent-orange/15 border border-accent-orange/30 hover:bg-accent-orange/25 hover:border-accent-orange/50 active:scale-95 transition-all duration-150 text-sm font-medium rounded-xl text-accent-orange">{installed ? "✅ Copiato!" : "📥 Installa"}</button>
        ...
        <div className="flex flex-col items-center pb-4 animate-fade-scale-in relative">
          ...
          <div className="flex md:hidden items-center gap-2 flex-wrap justify-center">
            ...
            <button onClick={async () => { await navigator.clipboard.writeText(p.urlPattern); setInstalled(true); setTimeout(() => setInstalled(false), 2000) }} disabled={!p.urlPattern} className="h-9 px-3 bg-accent-orange/15 border border-accent-orange/30 hover:bg-accent-orange/25 hover:border-accent-orange/50 active:scale-95 transition-all duration-150 text-[11px] font-medium rounded-xl text-accent-orange">{installed ? "✅ Copiato!" : "📥 Installa"}</button>
