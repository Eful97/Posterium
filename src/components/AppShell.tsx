    ...
    <div className="app-shell text-foreground relative overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-[1680px] mx-auto px-4 pt-5 md:pt-3 pb-20 md:pb-6 pb-[max(5rem,env(safe-area-inset-bottom)+4rem)]">
        ...
        <div className="flex flex-col items-center pb-4 animate-fade-scale-in relative">
          ...
          <img onClick={p.goHome} src="/posterium.svg" alt="Posterium" decoding="async" className="header-logo h-16 md:h-20 w-auto mb-6 md:mb-5 cursor-pointer hover:brightness-110 active:scale-95 transition-all duration-150" />
      ...
      <div className="fixed bottom-5 right-5 z-50 flex items-center gap-2 floating-group">
        <button
          ...
          className="h-9 w-9 flex items-center justify-center rounded-lg active:scale-90 transition-all duration-150 text-sm hover:bg-white/[0.08]"
        >
          <RefreshCw className={`w-4 h-4 ${refreshing ? "animate-spin" : ""}`} />
        </button>
        <a href="/status" aria-label={p.t("ui.statusTitle")} className="h-9 w-9 flex items-center justify-center rounded-lg active:scale-90 transition-all duration-150 text-sm hover:bg-white/[0.08]"><HeartPulse className="w-4 h-4" /></a>
        <div ref={p.langRef} className="relative">
          <button aria-label={p.t("ui.chooseLanguage")} onClick={() => p.setLangOpen((o) => !o)} className={`h-9 w-9 flex items-center justify-center rounded-lg active:scale-90 transition-all duration-150 text-sm ${p.langOpen ? "dropdown-open" : "hover:bg-white/[0.08]"}`} title={LANG_NAMES[p.lang]}>{LANG_FLAGS[p.lang] || <Globe className="w-4 h-4" />}</button>
          {p.langOpen && (
            <div className="absolute right-0 bottom-full mb-3 bg-black/60 backdrop-blur-xl border border-zinc-700/50 rounded-xl p-2 shadow-2xl shadow-black/50 z-50 min-w-40 animate-fade-scale-in dropdown-open">
