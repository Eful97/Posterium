...
import { RefreshCw, Settings, Globe, HeartPulse, Moon, Sun } from "lucide-react"
...
return (
    <div className={`app-shell text-foreground relative overflow-x-hidden ${p.theme === "light" ? "light" : ""}`}>
      ...
      <div className="relative z-10 max-w-[1680px] mx-auto px-4 pt-5 md:pt-3 pb-20 md:pb-6 pb-[max(5rem,env(safe-area-inset-bottom)+4rem)]">
        <div className="hidden md:flex justify-end items-center gap-2 mb-4">
          ...
          <div className="relative" ref={p.settingsRef}>
            <button aria-label={p.t("ui.settings")} onClick={(e) => { e.stopPropagation(); p.setSettingsOpen((o) => !o) }} className="top-action-button px-3 text-sm font-semibold flex items-center gap-2 bg-white/[0.06] border border-white/10 text-zinc-200">{p.t("ui.settings")}</button>
        ...
        <div className="flex flex-col items-center pb-4 animate-fade-scale-in relative">
          ...
          <div className="flex md:hidden items-center gap-2 flex-wrap justify-center">
            <button aria-label={p.t("ui.copyUrl")} onClick={() => { p.copyUrl() }} disabled={!p.urlPattern} className="top-action-button top-action-button-primary h-11 px-3 text-xs font-semibold bg-accent-orange text-white border border-accent-orange/50 shadow-lg shadow-accent-orange/25 disabled:opacity-40">{p.copied ? p.t("ui.copied") : p.t("ui.copyUrl")}</button>
            ...
            <button aria-label={p.t("ui.settings")} onClick={() => p.setSettingsOpen(true)} className="top-action-button h-11 w-11 flex items-center justify-center text-sm bg-white/[0.06] border border-white/10 text-zinc-200"><Settings className="w-5 h-5" /></button>
          </div>
          </>
        </div>
      ...
      <a href="/status" aria-label={p.t("ui.statusTitle")} className="floating-action fixed bottom-5 right-[4rem] z-50 h-11 w-11 flex items-center justify-center rounded-xl active:scale-90 transition-all duration-150 text-sm"><HeartPulse className="w-5 h-5" /></a>
      <button
        aria-label={p.theme === "dark" ? "Switch to light mode" : "Switch to dark mode"}
        onClick={() => p.setTheme(p.theme === "dark" ? "light" : "dark")}
        className="floating-action fixed bottom-5 right-[1rem] z-50 h-11 w-11 flex items-center justify-center rounded-xl active:scale-90 transition-all duration-150 text-sm bg-white/[0.06] border border-white/10 text-zinc-200"
        title={p.theme === "dark" ? "Tema chiaro" : "Tema scuro"}
      >
        {p.theme === "dark" ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
      </button>
      <div className="fixed bottom-5 right-5 z-50" ref={p.langRef}>
        <button aria-label={p.t("ui.chooseLanguage")} onClick={() => p.setLangOpen((o) => !o)} className="floating-action h-11 w-11 flex items-center justify-center rounded-xl active:scale-90 transition-all duration-150 text-sm" title={LANG_NAMES[p.lang]}>{LANG_FLAGS[p.lang] || <Globe className="w-5 h-5" />}</button>
        {p.langOpen && (
      ...
      {p.settingsOpen && (
        <div className="fixed inset-0 z-[70] bg-background md:hidden animate-fade-scale-in overflow-y-auto">
          ...
          <div className="flex items-center gap-3 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] border-b border-zinc-800">
            <button aria-label={p.t("ui.back")} onClick={() => p.setSettingsOpen(false)} className="text-sm text-zinc-300 hover:text-white active:scale-90 transition-all duration-150">{p.t("ui.back")}</button>
