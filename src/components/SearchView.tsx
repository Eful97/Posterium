  ...
  return (
    <div>
      <div className="max-w-lg mx-auto relative z-[100] isolate mb-8">
        <SearchBar tmdbKey={tmdbKey} value={query} onChange={setQuery} onSearch={(q) => { setQuery(q); doSearch(q) }} large onFocus={() => setSearchFocused(true)} onBlur={() => setTimeout(() => setSearchFocused(false), 200)} />
        {showRecent && (
      ...
      {results.length > 0 && (
        <div className="relative animate-fade-scale-in">
          {searching && <div className="absolute inset-0 bg-background/60 backdrop-blur-sm z-10 rounded-2xl flex items-center justify-center"><p className="text-sm text-zinc-400 animate-pulse">Ricerca in corso...</p></div>}
          <div className="mx-auto grid grid-cols-3 sm:grid-cols-[repeat(auto-fill,minmax(180px,1fr))] lg:grid-cols-5 gap-3 sm:gap-4 max-w-7xl justify-items-center">
          {results.map((r) => {
            ...
            return (
              <button key={`${r.media_type}:${r.id}`} onClick={() => navigateToPoster(r)} className="group relative bg-surface rounded-xl overflow-hidden border border-zinc-800 hover:border-accent/50 hover:-translate-y-1 hover:shadow-xl hover:shadow-accent/10 transition-all duration-200 ease-out w-full max-w-[250px] lg:max-w-none">
                ...
                {mapping && <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-accent flex items-center justify-center text-[10px] font-bold text-black shadow-lg shadow-accent/30" title="Custom poster set">✓</div>}
                {mapping && <div className="absolute inset-0 bg-accent/10 border border-accent/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl pointer-events-none" />}
              </button>
            )
          })}
          </div>
          {searchPage < totalPages && !searching && (
            <div className="flex justify-center mt-6">
              <button onClick={loadMore} className="px-6 py-3 rounded-xl text-sm font-medium bg-zinc-800 border border-zinc-700 hover:border-accent/50 hover:text-accent active:scale-95 transition-all duration-150">Mostra più risultati</button>
            </div>
          )}
        </div>
      )}
