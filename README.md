...
- **Warmup**: `/api/warmup` segue il token admin (`POSTERIUM_ADMIN_TOKEN`/`ADMIN_TOKEN`): **fail-open** solo su istanza pubblica esplicita (`POSTERIUM_PUBLIC_INSTANCE=1`) senza token (es. HF Spaces), **fail-closed** altrimenti. Opzionalmente puoi richiedere un **token dedicato** (`POSTERIUM_WARMUP_TOKEN`, header `x-warmup-token`) su istanze condivise: se impostato, vince sull'admin token e l'endpoint resta chiuso a chiunque non lo conosca. In più è rate-limited (bucket `warmup`) e protetto da CSRF (`isSameOrigin`), così chiunque non possa triggerare carico in loop. Il self-fetch dei poster usa un origin interno fisso (`127.0.0.1`), mai l'host derivato dagli header di richiesta (anti-SSRF). **Self-warmup post-deploy**: l'entrypoint Docker/HF lancia `/api/warmup` automaticamente in background dopo il boot (poll su `/api/health` fino a ~60s, fallimenti non fatali) — la cache poster è in-memory e ogni restart parte a freddo. Disattivabile con `POSTERIUM_SELF_WARMUP=0`.
...
| `POSTERIUM_WARMUP_TOKEN` | ❌ | Token dedicato per `/api/warmup` (header `x-warmup-token`). Se impostato, il warmup richiede questo token o l'admin token — su istanze condivise evita che l'endpoint-amplificatore sia triggerabile da chiunque (l'entrypoint lo invia automaticamente se configurato) |
